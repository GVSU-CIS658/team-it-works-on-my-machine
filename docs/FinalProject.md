...
Firebase Authentication manages user identity. Firebase Cloud Functions provide backend-mediated operations for sensitive user profile writes. Cloud Firestore stores application data, including user profiles, tasks, groups, sessions, and group feed records.
...
| `createTask` | Creates a private task for the signed-in user |
| `updateTask` | Updates a private task owned by the signed-in user |
| `deleteTask` | Deletes a private task owned by the signed-in user |
...
| Callable Function | Description |
| --- | --- |
| `createTask` | Creates a private task owned by the signed-in user |
| `updateTask` | Updates an existing private task owned by the signed-in user |
| `deleteTask` | Deletes an existing private task owned by the signed-in user |
...
Callable functions were selected because the project already uses Firebase Authentication and Cloud Firestore. Callable functions automatically receive Firebase Auth context from signed-in users, reducing the need for custom token parsing middleware. This keeps the backend service layer smaller while still enforcing backend-mediated writes for sensitive profile operations.
...
Current shared group feed shape:
...
Current `groupFeed` rule:

```js
match /groupFeed/{postId} {
...
Task and Groups backend integration followed:

9. Task create, update, and delete were moved to Cloud Functions.
10. Task reads remained in Firestore with owner-only rules.
11. Group create, join, leave, delete, and feed post creation were added as Cloud Functions.
...
This decision was made because the project already uses Firebase Authentication and Cloud Firestore. Callable Cloud Functions provide a backend service layer while integrating directly with Firebase Auth context and Firebase Admin SDK.
...
Callable functions still provide a backend service layer. They are invoked through the Firebase Functions SDK and run in a trusted server environment. This approach avoids manually passing and verifying ID tokens in custom Express middleware.
...
## 10.5 Join Codes and Membership

Groups use backend-generated join codes rather than user-entered join codes at creation time. This keeps the create-group flow simple and allows the backend to own uniqueness and membership assignment.

Group ownership and membership are represented by:
